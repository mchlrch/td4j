...
public class Workbench extends JFrame {
	...
	public static Workbench getInstance() {
		if (INSTANCE == null) throw new IllegalStateException("no workbench instance available. Try Workbench.start()");
	...
	public static void start(final Object initialNavigation, final Class<?>... sidebarEntries) {
		...
		final CompositeFormFactory formFactory = new CompositeFormFactory(byClassNameFormFactory, genericFormFactory);		
		
		final IEditorFactory editorFactory = new GenericEditorFactory(modelInspector, formFactory, connectorFactory);
		start(editorFactory, initialNavigation, sidebarEntries);
	}

	public static void start(final IEditorFactory editorFactory, final Object initialNavigation, final Class<?>... sidebarEntries) {
	...
	private static Workbench setup(final IEditorFactory editorFactory, final List<Class<?>> sidebarEntries) {
		if (INSTANCE == null) {
	...
	private Workbench(final IEditorFactory editorFactory, final List<Class<?>> sidebarEntries) {
		if (editorFactory == null) throw new NullPointerException("editorFactory");
	...
	private Component createSidebar(final SidebarModel model) {
		...
		final ICollectionDataConnector classOptionsConnector = connectorFactory.createCollectionFieldConnector(SidebarModel.class, "currentClassOptions");
		final ListController classOptionsController = new ListController(classChooser, classOptionsConnector.createProxy());

		// choice
		final IScalarDataConnector currentClassConnector = connectorFactory.createScalarMethodConnector(SidebarModel.class, "currentClass");
		final SelectionController currentClassController = new SelectionController(classChooser.getSelectionModel(), new ListModelAdapter(classChooser.getModel()), currentClassConnector.createProxy());
	...
	private void cacheVisibleModel() {
		...
		if (visibleClass == null || visibleEditor == null) return;
	...
	protected Editor editor(Class<?> cls) {
		...
		if (editor == null) {
			...
			editorPanel.add(editorComponent, new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0, GridBagConstraints.NORTHWEST, GridBagConstraints.BOTH, new Insets(5, 5, 5, 5), 0, 0));
			
//			editorPanel.add(editorComponent, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.NORTHWEST, GridBagConstraints.BOTH, new Insets(5, 5, 5, 5), 0, 0));
//			editorPanel.add(new JLabel(), new GridBagConstraints(0, 1, 1, 1, 1.0, 1.0, GridBagConstraints.NORTHWEST, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
	...
	public static class SidebarModel extends Observable {
		...
		public void setCurrentClass(Class<?> currentClass) {
			if (ObjectTK.equal(this.currentClass, currentClass)) return;
		...
		private void setCurrentClass0(Class<?> currentClass) {
			final ChangeEvent changeEvent = changeSupport.preparePropertyChange("currentClass", this.currentClass, currentClass);
	...
	public static void main(String[] args) {
		...
		start(homer, Person.class, Address.class);
	}
